...
# Bot init
bot = Bot(token=config.TLG_TOKEN)
...
async def start(message: types.Message):
    ...
    if users_in_table(user_id) is True:
        ...
        await message.answer(config.UNKNOWN_USER)


class DelFormId(StatesGroup):
    waiting_for_form_id = State()
    waiting_for_number_of_month = State()
...
async def show_help(message: types.Message):
    ...
    if users_in_table(user_id) is True:
        ...
        await DelFormId.waiting_for_number_of_month.set()
    else:
        await message.answer(config.UNKNOWN_USER)


@dp.message_handler(state=DelFormId.waiting_for_number_of_month)
async def form_id_start_chosen(message: types.Message, state: FSMContext):
    user = types.User.get_current()
    user_id = user.id
    full_name = user.full_name
    ...
    if not month_number.isdigit() or not 1 <= int(month_number) <= 12:
        ...
        config.cursor.execute(command_admin)
        records = config.cursor.fetchall()
        for row in records:
            await message.answer(f"Месяц: {config.MONTH[int(month_number)]}\n"
                                 f"{row[0]}\n"
        ...
        records = config.cursor.fetchone()
        await state.reset_data()
        await state.finish()
        await message.answer(f"Статистика:\n"
                             f"{full_name}\n"
                             f"Месяц: {config.MONTH[int(month_number)]}\n"
...
async def form_id_start(message: types.Message):
    user = types.User.get_current()
    user_id = user.id
    if users_in_table(user_id) is True:
        await message.answer("Введите id анкеты, которую хотите удалить")
        await DelFormId.waiting_for_form_id.set()
    else:
        await message.answer(config.UNKNOWN_USER)


@dp.message_handler(state=DelFormId.waiting_for_form_id)
async def form_id_start_chosen(message: types.Message, state: FSMContext):
    user = types.User.get_current()
    user_id = user.id
        ...
        if not form_id.isdigit():
            ...
            if double_form_id(form_id) is not False:
                ...
                if 194342 < int(form_id) <= 999999:
                    command = f"{config.INSERT_FORM_ID} " \
